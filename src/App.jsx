...
function neutralizePreScoreCopy(copy = '') {
  return copy
    ...
    .replace(/\bWrong:?\s*/gi, '')
    .replace(/\bis unrealistic and prohibitively expensive\b/gi, 'has major feasibility and cost implications')
    .replace(/\bis prohibitively expensive\b/gi, 'has major cost implications')
    .replace(/\bis unrealistic\b/gi, 'has feasibility constraints')
    .replace(/\bis not feasible\b/gi, 'has feasibility constraints')
    .replace(/\bImpossible\b/g, 'Has major feasibility constraints')
    .replace(/\bimpossible\b/g, 'has major feasibility constraints')
    .replace(/\bis irresponsible\b/gi, 'carries serious governance exposure')
    .replace(/\bis ethically unacceptable\b/gi, 'creates serious ethical exposure')
    .replace(/\bis illegal and unacceptable\b/gi, 'creates legal and governance exposure')
    .replace(/\bis illegal\b/gi, 'creates legal exposure')
    .replace(/\bis unacceptable\b/gi, 'creates serious exposure')
    .replace(/\bis not acceptable\b/gi, 'creates serious exposure')
    .replace(/\bis too risky\b/gi, 'carries high residual exposure')
    .replace(/\bis dangerous\b/gi, 'creates major continuity exposure')
    .replace(/\bis acceptable only when\b/gi, 'requires')
    .replace(/\bis acceptable only where\b/gi, 'requires')
    .replace(/\bare acceptable only when\b/gi, 'require')
    .replace(/\bare tolerable when\b/gi, 'require')
